...
var SSBPError = function() {};
SSBPError.BUSY = 1;
SSBPError.INVALID = 2;
SSBPError.CANCELED = 3;
SSBPError.TIMEOUT = 4;
SSBPError.UNKNOWN_ERROR = 5;
SSBPError.NOT_ACCEPT = 6;
...
/////////////////////////
//SSBP
var _SSBP = function(config, queue) {
	this.config = config;
	this.isExecute = false;
	this.queue = queue;
	this.success = null;
	...
	//ダウンロード
	this.download = function(options, successCallback, errorCallback, progressCallback){
		if(this.isExecute){
			errorCallback({code:SSBPError.BUSY, message:"BUSY 1"});
			return;
		}
		this.isExecute = true;
		...
		if(applican.config.debug){
			...
			result.totalTime = 10;
			result.localPath = "file:///xxx/yyy/zzz.dat";
			successCallback(result);
		}else{
			var scheme = 'applican-api://ssbp/download/'+encodeURIComponent(JSON.stringify(options));
			this.queue.pf_callApi(scheme);
		}
	};
	this._downloadSuccess = function(result){
		this.isExecute = false;
		this.success(result);
	};
	this._downloadError = function(error){
		this.isExecute = false;
		this.error(error);
	};
	...
	//アップロード
	this.upload = function(options, successCallback, errorCallback, progressCallback){
		if(this.isExecute){
			errorCallback({code:SSBPError.BUSY, message:"BUSY"});
			return;
		}
		this.isExecute = true;
		...
		if(applican.config.debug){
			...
			result.totalBytes = 1000;
			result.totalTime = 10;
			result.remotePath = "zzz.dat";
			successCallback(result);
		}else{
			var scheme = 'applican-api://ssbp/upload/'+encodeURIComponent(JSON.stringify(options));
			this.queue.pf_callApi(scheme);
		}
	};
	this._uploadSuccess = function(result){
		this.isExecute = false;
		this.success(result);
	};
	this._uploadError = function(error){
		this.isExecute = false;
		this.error(error);
	};
	this._uploadProgress = function(progress){
		this.progress(progress);
	};
	...
	this.cancel = function(){
		if(applican.config.debug){
			...
			var scheme = 'applican-api://ssbp/cancel/';
			this.queue.pf_callApi(scheme);
		}
	}
};
...
var applicanRoot = function() {
	...
	this.ssbp = new _SSBP(this.config, this.queue);
